    ...
    def __init__(self, app):
        ...
        for level_line in levels.split("\n"):
            ...
            try:
                ...
                L.error(
                    "Cannot read line '{}' in '[logging] levels' section, expected format: 'logger_name level_name'.".format(
                        level_line
                    )
                )
                continue
...
class AsyncIOHandler(logging.Handler):
    ...
    def _on_read(self):
        try:
            ...
            print(
                "Error on the syslog socket '{}'".format(self._address),
                e,
                file=sys.stderr,
            )
        ...
        try:
            ...
            if self._write_ready:
                try:
                    ...
                    print(
                        "Error when writing to syslog '{}'".format(self._address),
                        e,
                        file=sys.stderr,
                    )
                    self._enqueue(msg)
            ...
            print(
                "Error when emit to syslog '{}'".format(self._address),
                e,
                file=sys.stderr,
            )
