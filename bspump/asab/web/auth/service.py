...
from bspump.asab import Config, Service
...
class AuthService(Service):
    ...
    def __init__(self, app, service_name="asab.AuthzService"):
        ...
        self.MultitenancyEnabled = Config.getboolean("auth", "multitenancy")
        self.PublicKeysUrl = Config.get("auth", "public_keys_url")

        enabled = Config.get("auth", "enabled", fallback=True)
        if enabled == "mock":
            self.Mode = AuthMode.MOCK
        elif utils.string_to_boolean(enabled):
    ...
    def _prepare_mock_user_info(self):
        ...
        if not isinstance(resources, dict) or not all(
            map(
                lambda kv: isinstance(kv[0], str) and isinstance(kv[1], list),
                resources.items(),
            )
        ):
    ...
    async def _fetch_public_keys_if_needed(self, *args, **kwargs):
        ...
        if (
            self.AuthServerLastSuccessfulCheck is not None
            ...
            try:
                async with session.get(self.PublicKeysUrl) as response:
                    if response.status != 200:
                        L.error(
                            "HTTP error while loading public keys.",
                            struct_data={
                    ...
                    try:
                        ...
                        L.error(
                            "JSON decoding error while loading public keys.",
                            struct_data={
                                "url": self.PublicKeysUrl,
                                "data": data,
                            },
                    ...
                    try:
                        ...
                        L.error(
                            "Error while loading public keys: No public keys in server response.",
                            struct_data={
                                "url": self.PublicKeysUrl,
                                "data": data,
                            },
                ...
                L.error(
                    "Connection error while loading public keys: {}".format(e),
                    struct_data={
                        "url": self.PublicKeysUrl,
                    },
                )
    ...
    def _authorize_tenant_request(self, request, tenant):
        ...
        if tenant not in request._Tenants:
            L.warning(
                ...
                struct_data={"tenant": tenant, "sub": request._UserInfo.get("sub")},
            )
