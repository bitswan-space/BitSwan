...
class WinRMSource(TriggerSource):
    ...
    def __init__(self, app, pipeline, id=None, config=None):
        ...
        if os.path.exists(self.LastValueStorage) and not os.path.isdir(
            self.LastValueStorage
        ):
    ...
    async def cycle(self, *args, **kwags):
        ...
        while not command_done:
            (
                ...
                status_code,
                command_done,
            ) = self.Protocol.get_command_output_nowait(self.ShellId, command_id)

            # Log errors
            if std_err is not None and len(std_err) > 0:
                L.error(
                    "Error occurred in WinRMSource for command '{}' with status code '{}': '{}'".format(
            ...
            if std_out is not None and len(std_out) > 0:
                ...
                for line in lines:
                    if len(line) > 0:
                        await self.process(line, context={"status_code": status_code})
                        self.LastValue = line
...
class WinRMProtocol(winrm.protocol.Protocol):
    ...
    def get_command_output_nowait(self, shell_id, command_id):
        ...
        return_code = None
        try:
            stdout, stderr, return_code, command_done = self._raw_get_command_output(
                shell_id, command_id
            )
