    ...
    def set_row(self, context, event):
        ...
        for key in event.keys():
            ...
            if field_value is None:
                ...
                frac = int(
                    (field_value - int(field_value)) * 10000
                )  # The fraction of a second as one tenth of a millisecond (1/10000)
                row.setDateTime(
                    self.ColumnMapping[key],
                    t_t[0],
                    t_t[1],
                    t_t[2],
                    t_t[3],
                ...
                seconds = minutes * 60
                row.setDuration(
                    self.ColumnMapping[key], days, hours, minutes, seconds, frac
                )
            elif field_type == "integer":
    ...
    def process(self, context, event):
        if self.DataExtract is None:
            if not os.path.isfile(self.get_file_name(context, event)):
                # create table
                self.DataExtract = tableausdk.Extract.Extract(
                    self.get_file_name(context, event)
                )
                ...
                self.Table = self.DataExtract.addTable(
                    self.Config["table_name"], self.DataSchema
                )
            else:
