...
class MyPipeline(bspump.Pipeline):
    def __init__(self, app, pipeline_id=None):
        ...
        model = MyLSTMModel(
            ...
            id="MyModel",
            config={
                "path_model": "examples/timeseries/my_model.h5",
                "path_parameters": "examples/timeseries/my_parameters.json",
            },
        )
        self.build(
            bspump.file.FileCSVSource(
                app, self, config={"path": "examples/timeseries/data.csv"}
...
class MyTimeSeriesPredictor(bspump.timeseries.TimeSeriesPredictor):
    ...
    def alarm(self):
        error = tf.keras.metrics.mean_absolute_error(
            self.TimeWindow.Array[0, :]["value"],
            self.TimeWindow.Array[0, :]["predicted"],
        ).numpy()
