...
class MasterApplication(bspump.BSPumpApplication):
    def __init__(self):
        ...
        matrix = bspump.analyzer.SessionMatrix(
            self,
            dtype=[
                ("ts_start", "i8"),
                ("ts_end", "i8"),
        ...
        lookup = MyMatrixLookup(
            self, matrix_id="MySessionMatrix", on_clock_update=True, id="MyLookup"
        )
    ...
    def __init__(self, app, pipeline_id=None):
        ...
        self.build(
            bspump.file.FileCSVSource(
                ...
                self,
                config={
                    "path": "cz.o2tv.db.as.cac.live.epg.results.csv",
                    "delimiter": ";",
                },
            ).on(bspump.trigger.OpportunisticTrigger(app, chilldown_period=1)),
            bspump.random.RandomEnricher(
                app, self, choice=ranges, config={"field": "range"}, id="RE0"
            ),
            MyEnricher(app, self),
    ...
    def evaluate(self, context, event):
        ...
        event["ts_start"] = int(
            datetime.datetime.strptime(
                event["startdate"] + ":00", "%Y-%m-%d %H:%M:%S%z"
            ).timestamp()
        )
        event["ts_end"] = int(
            datetime.datetime.strptime(
                event["enddate"] + ":00", "%Y-%m-%d %H:%M:%S%z"
            ).timestamp()
        )
        self.Sessions.store_event(
            idx,
            event,
            keys=[
                "ts_start",
