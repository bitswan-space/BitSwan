...
class RandomSource(bspump.Source):
    async def main(self):
        while True:
            ...
            event = random.choice(["A", "B", "C"]) + "".join(
...
class CircuitBreaker(bspump.Processor):
    ...
    def set(self, value=False):
        ...
        if value:
            L.info(
                "Circuit breaker '{}' in a pipeline '{}' is on.".format(
                    self.Id,
                    self.Pipeline.Id,
                )
            )
            ...
            L.warning(
                "Circuit breaker '{}' in a pipeline '{}' is OFF.".format(
                    self.Id,
                    self.Pipeline.Id,
                )
            )
...
class TeePipeline(bspump.Pipeline):
    def __init__(self, app, pipeline_id):
        ...
        self.build(
            bspump.common.TeeSource(app, self).bind(
                "RandomSourcePipeline.TeeProcessor"
            ),
            bspump.common.PPrintSink(app, self),
        )
...
listen=127.0.0.1 8080
    ...
    # Register pipelines
    svc.add_pipelines(
        random_source_pipeline, target_pipeline_a, target_pipeline_b, target_pipeline_c
    )
    svc.add_pipeline(TeePipeline(app, "TeePipeline"))
