...
class MyTimeWindowAnalyzer(bspump.analyzer.TimeWindowAnalyzer):
    ...
    def __init__(self, app, pipeline, id=None, config=None):
        start_time = datetime.datetime(
            year=2020, month=8, day=31, hour=8, minute=0, second=0
        ).timestamp()
        super().__init__(
            columns=20,
            resolution=30,
    ...
    def evaluate(self, context, event):
        ...
        print(
            ">>> Item added to row: {}, column: {}, sum: {} ({})".format(
                row, column, self.TimeWindow.Array[row, column], str(event)
            )
        )
    ...
    def analyze(self):
        ...
        for row in range(0, len(x)):
            for column in range(0, len(x[row])):
                if x[row][column] > 2:
                    print(
                        "More than two logins at row '{}' and column '{}'.".format(
                            row, column
                        )
                    )
