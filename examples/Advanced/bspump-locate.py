...
class BasicPipeline(bspump.Pipeline):
    def __init__(self, app, pipeline_id):
        ...
        self.Source = bspump.file.FileCSVSource(
            app,
            self,
            config={"path": "./data/sample.csv", "delimiter": ";", "post": "noop"},
        )
...
if __name__ == "__main__":
    ...
    processor = svc.locate(
        "SamplePipeline.PPrintProcessor"
    )  # To locate a processor, use ID of the pipeline and processor separated by '.'.
    sink = svc.locate(
        "SamplePipeline.BlackHoleSink"
    )  # The same applies for the sink, since its class is derived from processor base class.
    source = svc.locate(
        "SamplePipeline.*FileCSVSource"
    )  # To locate a source, add '*' before the source id.
    non_existing = svc.locate(
        "SamplePipeline.NoSuchSinkExists"
    )  # If no such component exists, the method returns None.
