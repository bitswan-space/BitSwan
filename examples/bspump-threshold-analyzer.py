    ...
    def __init__(self, app, pipeline_id):
        ...
        self.build(
            ...
            bspump.random.RandomEnricher(
                app,
                self,
                config={"field": "server_load", "lower_bound": 1, "upper_bound": 1500},
                id="RE0",
            ),
            bspump.random.RandomEnricher(
                app,
                ...
                config={
                    "field": "@timestamp",
                    "lower_bound": int(
                        datetime.datetime.timestamp(datetime.datetime.now()) - 250
                    ),
                    "upper_bound": int(
            ...
            ExceedThresholdAnalyzer(
                app,
                self,
                config={
                    "event_attribute": "server",
                    "upper_bound": 1000,
                    "event_value": "server_load",
                    "symptom_occurrence": 3,
                    "analyze_period": 60,
                },
            ),
...
class ExceedThresholdAnalyzer(bspump.analyzer.ThresholdAnalyzer):
    ...
    def alarm(self, x, y, count, index):
        for c in range(0, count):
            L.warning(
                "Threshold has been exceeded on those positions: {}".format(
                    str([x[index - c], y[index - c]])
                )
            )
...
if __name__ == "__main__":
    ...
    svc.add_pipeline(CustomPipeline(app, "CustomPipeline"))
