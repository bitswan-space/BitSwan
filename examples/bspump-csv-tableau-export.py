...
class PrimaryPipeline(Pipeline):
    def __init__(self, app, pipeline_id=None):
        ...
        lb_0 = 0
        ub_0 = 3
        dtype = [("name", "U8"), ("fractions", "(2,2)f8"), ("num", "i8")]

        choice = ["a", "b", "c"]
        self.build(
            bspump.random.RandomSource(
                app, self, choice=choice, config={"number": 500}
            ...
            bspump.random.RandomEnricher(
                app,
                self,
                config={"field": "fraction", "lower_bound": lb_0, "upper_bound": ub_0},
                id="RE1",
            ),
            MySessionAnalyzer(app, self, dtype=dtype, analyze_on_clock=False),
            MyTimeWindowAnalyzer(
                app,
                self,
                tw_dimensions=(10, 1),
                resolution=60 * 60 * 24,
...
class SecondaryPipelineCSVTimeWindow(Pipeline):
    def __init__(self, app, pipeline_id=None):
        super().__init__(app, pipeline_id)
        self.build(
            bspump.matrix.MatrixSource(app, self, "MyTimeWindowAnalyzerMatrix").on(
                bspump.trigger.PubSubTrigger(app, "Export!")
            ),
...
class SecondaryPipelineCSVSession(Pipeline):
    def __init__(self, app, pipeline_id=None):
        super().__init__(app, pipeline_id)
        self.build(
            bspump.matrix.MatrixSource(app, self, "MySessionAnalyzerMatrix").on(
                bspump.trigger.PubSubTrigger(app, "Export!")
            ),
