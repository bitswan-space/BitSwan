...
class MyPipeline(bspump.Pipeline):
    def __init__(self, app, pipeline_id=None):
        ...
        model = MyRFModel(
            ...
            id="MyModel",
            config={
                "path_model": "examples/classification/MBDomainsModel.pkl",
        ...
        self.build(
            bspump.file.FileLineSource(
                app,
                self,
                config={"path": "examples/classification/named.log", "post": "move"},
...
class NamedParser(bspump.Processor):
    ...
    def process(self, context, event):
        ...
        pattern = re.compile(
            r"^(\S+.\S+)\squeries:\s(\w+):\s\w+\s(\S+)\s(\S+)\s\S+\squery:\s(\S+)\s\w+\s(\w+).+$"
        )
        ...
        parsed_event["timestamp"] = int(
            (
                datetime.datetime.strptime(event_split[1], "%d-%B-%Y %H:%M:%S.%f")
            ).timestamp()
        )
...
class MyRFModel(bspump.model.Model):
    ...
    def transform(self, sample):
        ...
        df["WHOIS_REGDATE"] = pd.to_datetime(
            df.WHOIS_REGDATE, errors="coerce", utc=True
        )
        df["WHOIS_UPDATED_DATE"] = pd.to_datetime(
            df.WHOIS_UPDATED_DATE, errors="coerce", utc=True
        )
        df["WHOIS_UPDATED_DATE"] = (
            df["WHOIS_UPDATED_DATE"] - pd.to_datetime("1/1/1970", utc=True)
        ) / np.timedelta64(1, "D")
        df["WHOIS_REGDATE"] = (
            df["WHOIS_REGDATE"] - pd.to_datetime("1/1/1970", utc=True)
        ) / np.timedelta64(1, "D")
