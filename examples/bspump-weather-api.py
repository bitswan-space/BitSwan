        ...
        async with aiohttp.ClientSession() as session:
            ...
            for city in self.cities:
                async with session.get(
                    url=f"https://api.openweathermap.org/data/2.5/weather?q={city}&units=metric&appid={self.Config['api_key']}"
                ) as response:
...
class SamplePipeline(bspump.Pipeline):
    def __init__(self, app, pipeline_id):
        ...
        self.build(
            LoadSource(app, self).on(bspump.trigger.PeriodicTrigger(app, 5)),
